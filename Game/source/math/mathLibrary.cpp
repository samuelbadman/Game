...
double mathLibrary::fRoundTo1DecimalPlace(const double value, const bool negativeAwayFromZero)
{
    return static_cast<double>(static_cast<int64_t>(value * 10 + (0.5 * (negativeAwayFromZero ? sign(value) : 1.0)))) / 10;
}

double mathLibrary::fRoundTo2DecimalPlace(const double value, const bool negativeAwayFromZero)
{
    return static_cast<double>(static_cast<int64_t>(value * 100 + (0.5 * (negativeAwayFromZero ? sign(value) : 1.0)))) / 100;
}

double mathLibrary::fRoundTo3DecimalPlace(const double value, const bool negativeAwayFromZero)
{
