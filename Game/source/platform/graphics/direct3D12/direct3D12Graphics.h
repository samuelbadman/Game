...
public:
	...
	static void render(const uint32_t numSurfaces, const class graphicsSurface* const* surfaces, const bool useVSync, const uint32_t meshCount, const struct sMeshResources* const* meshes);
	static void loadMesh(const size_t vertexCount, const struct sVertexPos3Norm3Col4UV2* const vertices, const size_t indexCount, const uint32_t* const indices,
...
private:
	...
	static void recordSurface(const class graphicsSurface* surface, ID3D12GraphicsCommandList6* commandList, const uint32_t meshCount, const struct sMeshResources* const* meshes);
